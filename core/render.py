...
class Renderer:
    ...
    GLASS_MARGIN = 36
    """玻璃卡片外边距（露出背景的区域）"""
    GLASS_PANEL_PADDING = 14
    """玻璃面板相对内容的外扩距离"""
    GLASS_RADIUS = 28
    """玻璃面板圆角半径"""
    GLASS_GRADIENT_TOP: ClassVar[Color] = (125, 160, 210)
    """默认渐变背景顶部色"""
    GLASS_GRADIENT_BOTTOM: ClassVar[Color] = (230, 175, 200)
    """默认渐变背景底部色"""
    ...
    async def _create_card_image(
        ...
        if not_repost:
            return self._compose_glass_card(image)
        return image

    def _resolve_bg_image_path(self) -> Path | None:
        """解析自定义背景图路径，支持绝对路径或相对插件数据目录的路径"""
        raw = (self.cfg.card_bg_image or "").strip()
        if not raw:
            return None
    ...
    @staticmethod
    def _cover_fit(img: PILImage, size: tuple[int, int]) -> PILImage:
        """等比缩放并居中裁剪，使图片完全覆盖目标尺寸"""
    ...
    def _make_gradient_background(self, size: tuple[int, int]) -> PILImage:
        """生成默认的竖向柔和渐变背景"""
        width, height = size
        ...
        for y in range(256):
            ...
            grad.putpixel(
                ...
                tuple(
                    round(a + (b - a) * ratio)
                    for a, b in zip(self.GLASS_GRADIENT_TOP, self.GLASS_GRADIENT_BOTTOM)
                ),
            )
        return grad.resize((width, height))

    def _load_glass_background(self, size: tuple[int, int]) -> PILImage:
        """加载玻璃卡片背景：优先自定义背景图（可配置模糊），失败或留空则用渐变"""
        if bg_path := self._resolve_bg_image_path():
            try:
                with Image.open(bg_path) as img:
                    bg = self._cover_fit(img.convert("RGB"), size)
            except Exception as e:
                logger.warning(f"加载卡片背景图失败: {e}, 回退到渐变背景")
                bg = self._make_gradient_background(size)
            else:
                blur_radius = self.cfg.card_bg_blur or 0
                if blur_radius > 0:
    ...
    def _compose_glass_card(self, content: PILImage) -> PILImage:
        """把透明底的内容画布合成到玻璃卡片上（背景 + 玻璃面板 + 内容）"""
        margin = self.GLASS_MARGIN
        width, height = content.size
        total_w, total_h = width + margin * 2, height + margin * 2

        # 背景层
        canvas = self._load_glass_background((total_w, total_h)).convert("RGBA")

        # 玻璃面板（圆角半透明白 + 高亮描边）
        opacity = max(10, min(100, self.cfg.card_glass_opacity or 70))
        fill_alpha = round(opacity / 100 * 255)
        border_alpha = min(255, fill_alpha + 55)
        panel = Image.new("RGBA", (total_w, total_h), (0, 0, 0, 0))
        pad = self.GLASS_PANEL_PADDING
